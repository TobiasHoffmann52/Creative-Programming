...
(function() {
  ...
  p5.prototype.fillGradient = function() {
    ...
    from:
      ...
      [ width, height ],
      steps:
      [ color(255), color(0, 96, 164), color(0) ]
    }
    ,
    radial:
    {
    from:
      [ width / 2, height / 2, 0 ],
      to:
      [ width / 2, height / 2, max(width / 2, height / 2) ],
      steps:
      [ color(255), color(0, 96, 164), color(0) ]
    }
    ,
  ...
  let _gradients = {
  linear:
  () => _ctx.createLinearGradient(_props.from[0], _props.from[1], _props.to[0], _props.to[1]),
  radial:
  () => _ctx.createRadialGradient(_props.from[0], _props.from[1], _props.from[2], _props.to[0], _props.to[1], _props.to[2]),
  conic:
  () => _ctx.createConicGradient(radians(_props.from[2]), _props.from[0], _props.from[1])
};
